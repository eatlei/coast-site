...
function Scan() {
  ...
  return (
    <div ref={root} className="relative h-11 w-full max-w-[200px] overflow-hidden rounded-[3px] border border-rule">
      <div className={`absolute left-2.5 top-1.5 ${mono} v`}>{t("楼下牛肉面", "Beef noodles")}</div>
...
function Reconcile() {
  ...
  // 三行放进 44px 的舞台：默认行高 18px 一共 54，底下那行会被裁掉（中英文都一样），收到 14px
  return (
    <div ref={root} className={`h-11 w-full max-w-[220px] leading-[14px] ${mono}`}>
      <div className="flex justify-between"><span className="text-muted-foreground">{t("校准", "Reconciled")}</span><span>¥12,340</span></div>
      <div className="stack flex justify-between text-muted-foreground"><span>{t("+ 流水", "+ entries")}</span><span>+ ¥1,220</span></div>
      <div className="stack flex justify-between"><span>{t("= 现在", "= now")}</span><span>¥13,560 <span className="ok text-success">✓</span></span></div>
    </div>
  )
}
...
function Recurring() {
  ...
  return (
    <div ref={root} className={`relative h-11 w-full max-w-[220px] ${mono}`}>
      ...
      <div className="one absolute inset-x-0 top-0 flex justify-between"><span>Netflix</span><span className="text-primary">{t("每月 ×3 ✓", "monthly ×3 ✓")}</span></div>
    </div>
  )
}
...
function Rules() {
  ...
  return (
    <div ref={root} className={`h-11 w-full max-w-[220px] ${mono}`}>
      <div className="flex gap-2"><span>{t("美团外卖", "Uber Eats")}</span><span className="text-muted-foreground">→</span><span className="cat">{t("弹性 · 外食", "Flexible · Dining")}</span></div>
      <div className="ask mt-1 text-muted-foreground">{t("下次自动这样归类？ 记住", "Always file it here? Remember")}</div>
    </div>
  )
}
...
function Review() {
  ...
  return (
    <div ref={root} className="relative h-11 w-full max-w-[220px]">
      ...
      <div className={`say absolute inset-x-0 top-0 ${mono}`}>{t("外食比上月多了 ¥620，占了超支的大头。", "Dining is up ¥620 on last month — most of the overspend.")}</div>
    </div>
  )
}
...
function Repeat() {
  ...
  return (
    <div ref={root} className={`h-11 w-full max-w-[220px] ${mono}`}>
      ...
      <div className="sum flex justify-between text-muted-foreground"><span>{t("一年", "a year")}</span><span>≈ ¥3,300</span></div>
    </div>
  )
}
...
function Scenarios() {
  ...
  return (
    <div ref={root} className="relative h-11 w-full max-w-[220px]">
      ...
      <span className={`lbl absolute right-9 top-0 ${mono} text-primary`}>{t("换城市", "Move")}</span>
      <span className={`lbl absolute right-0 bottom-2 ${mono} text-muted-foreground`}>{t("不变", "Stay")}</span>
    </div>
  )
}
...
function Widgets() {
  ...
  return (
    <div ref={root} className={`grid h-11 w-full max-w-[240px] grid-cols-3 gap-1.5 ${mono}`}>
      <div className="w rounded-[3px] border border-rule px-1.5 py-1 leading-tight"><div className="text-[9px] text-muted-foreground">{t("还能花", "Left")}</div>¥2,585</div>
      <div className="w rounded-[3px] border border-rule px-1.5 py-1 leading-tight"><div className="text-[9px] text-muted-foreground">{t("净资产", "Net worth")}</div>{t("57.6万", "576K")}</div>
      <div className="w rounded-[3px] border border-rule px-1.5 py-1 leading-tight"><div className="text-[9px] text-muted-foreground">{t("自由日", "Free in")}</div>8y 7m</div>
    </div>
  )
}
...
function Currency() {
  ...
  return (
    <div ref={root} className={`h-11 w-full max-w-[220px] ${mono}`}>
      ...
      <div className="rate text-muted-foreground">{t("今日 7.124 · 自动", "today 7.124 · auto")}</div>
    </div>
  )
}
...
function Privacy() {
  ...
  return (
    <div ref={root} className={`relative h-11 w-full max-w-[220px] ${mono}`}>
      <div className="real"><div className="flex justify-between"><span>{t("本月支出", "This month")}</span><span>¥9,414.89</span></div><div className="flex justify-between text-muted-foreground"><span>{t("净资产", "Net worth")}</span><span>¥574,907</span></div></div>
      <div className="mask absolute inset-0"><div className="flex justify-between"><span>{t("本月支出", "This month")}</span><span>¥●,●●●.●●</span></div><div className="flex justify-between text-muted-foreground"><span>{t("净资产", "Net worth")}</span><span>¥●●●,●●●</span></div></div>
    </div>
  )
}
