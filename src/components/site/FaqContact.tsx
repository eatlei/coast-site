...
const FAQ = [
  ...
  ["能导入以前的账单吗？", "可以。支持 CSV / XLSX，UTF-8 和 GB18030 编码自动识别，常见记账 App、银行和支付平台导出的文件可以直接导入。格式对不上的（对账单 PDF、截图、自己的表格），导入页有一张模板和一段提示词：复制给 AI，让它按模板转一下再导入。账户名会模糊匹配，自动去重，整批可撤销。", "Can I import my old records?", "Yes. CSV / XLSX with automatic UTF-8 and GB18030 detection; exports from most budgeting apps, banks and payment platforms import directly. For anything else — statement PDFs, screenshots, your own spreadsheets — the import page has a template and a prompt: copy it to an AI, let it convert your data to the template, then import. Account names are fuzzy-matched, duplicates skipped, and the whole batch can be undone."],
